  ...
  describe('koto.Base', function () {
    ...
    describe('Attachments', function () {
      ...
      describe('#demux', function () {
        ...
        it('should use provided function to demultiplex the data', function (done) {
          ...
          this.myChart.draw(data).then(() => {
            expect(this.attachmentChart.draw.args, 'Demuxes data passed to charts with registered function').to.deep.equal([[[1, 2, 3]]]);

            expect(this.attachmentChart2.draw.args[0][0].series1, data.series1, 'Unmodified data passes through to attachments directly').to.deep.equal(data.series1);

            expect(this.attachmentChart2.draw.args[0][0].series2, data.series1, 'Unmodified data passes through to attachments directly').to.deep.equal(data.series2);
            done();
          }, throwError);
        });
    ...
    describe('#draw', function () {
      ...
      it('should return a promise with transformed data', function (done) {
        var data = [1, 2, 3];
        this.myChart.draw(data).then(transformed => {
